...
const gameData = {paused:true, action:false, tubes:[], colorsArr:[], colorIndex:0, tubesArr:[], tubesArrIndex:0, stageNum:0, stage:{}, levelNum:0, challengeNum:0, tubeNum:0, complete:false, levelCompleted:1};
const tweenData = {score:0, tweenScore:0};
const timeData = {enable:false, startDate:null, sessionDate:null, nowDate:null, sessionTimer:0, timer:0, oldTimer:0, accumulate:0};
const selectData = {page:0, total:1, max:20, column:5, row:4};
const cookieName = 'liquidsort2022';
...
function goPage(page){
	...
	switch(page){
		...
		case 'result':
			...
			if(gameData.type == "level"){
				resultTitleTxt.text = textStrings.resultLevelTitle.replace("[NUMBER]", gameData.challengeNum);
			}else{
...
function prepareStage(){
	...
	gameData.tube = {w:0, h:0, colorW:0, fillW:0, fillH:0, fillX:0, fillY:0, fillPercent:0, pourW:10, pourH:0, surfaceScale:.3, hidden:levelSettings[gameData.levelNum].hidden};
...
function pushColours(index, color, height){
	...
	var hiddenColor = gameData.tube.hidden == true;
	thisLiquid.data.colors.push({shape:null, surface:null, bottom:null, index:color, height:height, hidden:hiddenColor});
}
...
function fillLiquid(index){
	...
	var pos = {x:0, y:0, w:gameData.tube.colorW, hiddenIndex:0};
	for(var n=0; n<thisLiquid.data.colors.length; n++){
		var colorFill = colors_arr[thisLiquid.data.colors[n].index].fill;
		var colorSurface = colors_arr[thisLiquid.data.colors[n].index].surface;
		if(n == thisLiquid.data.colors.length-1){
			thisLiquid.data.colors[n].hidden = false;
		}
		if(thisLiquid.data.colors[n].hidden){
			colorFill = colorsHidden_arr[pos.hiddenIndex].fill;
			colorSurface = colorsHidden_arr[pos.hiddenIndex].surface;
			pos.hiddenIndex++;
			pos.hiddenIndex = pos.hiddenIndex > colorsHidden_arr.length-1 ? colorsHidden_arr.length-1 : pos.hiddenIndex;
		}
		...
		thisArray.shape.graphics.beginFill(colorFill).drawRect(-(pos.w/2), thisArray.shape.extraH, (pos.w), -(thisArray.height+thisArray.shape.extraH));
		...
		thisArray.surface.graphics.beginFill(colorSurface).drawCircle(0, 0, gameData.tube.fillW/2);
		thisArray.surface.color = thisLiquid.data.colors[n].index;

		thisArray.bottom = new createjs.Shape();
		thisArray.bottom.graphics.beginFill(colorFill).drawCircle(0, 0, gameData.tube.fillW/2);
	...
	for(var n=0; n<gameData.tubes.length; n++){
		...
		for(var c=0; c<gameData.tubes[n].data.colors.length; c++){
			...
			if(gameData.tubes[n].data.fill >= gameData.tube.fillH - 5){
				if(checkColor == -1 && !gameData.tubes[n].data.colors[c].hidden){
					checkColor = gameData.tubes[n].data.colors[c].index;
				}else if(checkColor != gameData.tubes[n].data.colors[c].index || gameData.tubes[n].data.colors[c].hidden){
					completeColor = false;
				}
			}else{
				completeColor = false;
	...
	if(leftTubesArr.length == 2 && gameSettings.autoFill){
		...
		for(var n=0; n<leftTubesArr.length; n++){
			if(leftTubesArr[n].data.colors.length == 1){
				if(tubeColor == -1 && !leftTubesArr[n].data.colors[0].hidden){
...
function calculateScore(){
	...
	TweenMax.to(timeData, 1, {sessionTimer:0, overwrite:true, onUpdate:function(){
		...
		statusTxt.text = textStrings.score.replace("[NUMBER]", addCommas(totalScore));
		updateTimerBar();
	}, onComplete:function(){
		var calTimer = timeData.countdown - timeData.sessionTimer;
		var totalScore = Math.floor((calTimer - timeData.accumulate) * scorePercentage);
...
function updateGameLevel(){
	levelTxt.text = textStrings.level.replace("[NUMBER]", gameData.challengeNum);
}

function showGameStatus(con){
	var delay = 2;
	if(con == 'timesup'){
		statusTxt.text = textStrings.timesup;
	}else if(con == 'clear'){
		if(gameData.type == "level"){
			statusTxt.text = textStrings.clear.replace("[NUMBER]", gameData.levelNum+1);
		}else{
			statusTxt.text = textStrings.clear.replace("[NUMBER]", gameData.challengeNum);
		}
	}
